...
def delta_hedge_sim(S_paths, v_paths, times, K, r, q, tc=0.0008, impact_lambda=0.0, option_type='call', rebal_freq=1, deltas_mode='bs', per_path_deltas=None):
    ...
    for i in range(n_paths):
        ...
        for k in range(1, len(reb)):
            ...
            cost = torch.abs(trade_value) * tc + impact_lambda * (trade_value**2)
            cash -= trade_value + cost
            Delta_prev = Delta_new
        
        # Handle expiry (improved accuracy for ITM/OTM distinction)
        for j in range(int(reb[-1]), n_steps): 
            cash *= torch.exp(r*dt[j])
        
        # At expiry, delta is 1 if ITM, 0 if OTM for calls (exact handling)
        if option_type == 'call':
            Delta_expiry = (S_path[-1] > K).float()
            payoff = torch.maximum(S_path[-1] - K, torch.tensor(0.0, device=device))
        else:
            Delta_expiry = (S_path[-1] < K).float()  # For puts
            payoff = torch.maximum(K - S_path[-1], torch.tensor(0.0, device=device))
        
        # Final hedge adjustment if needed
        final_hedge_cost = torch.abs((Delta_expiry - Delta_prev) * S_path[-1]) * tc
        cash -= final_hedge_cost
